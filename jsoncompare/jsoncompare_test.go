...
var JSON4 []byte = []byte(`{"family":{"papa":"yes","mama":"too","children":{"number":3,"list":[{"type":"boy","name":"John"},{"type":"girl","name":"Linda"},{"hhhhh": "{\"type\":\"boy\",\"name\":\"Mike\"}"}]}}}`)
...
//	//c.Skip("Not now")
//	p1 := []path{}
//	p1 = append(p1, path{path: "1", valueType: "map", value: 11})
//	p1 = append(p1, path{path: "2", valueType: "slice", value: 22})
//	p1 = append(p1, path{path: "3", valueType: "int64", value: 45})
//	p1 = append(p1, path{path: "4", valueType: "string", value: "www"})
//	p1 = append(p1, path{path: "5", valueType: "<nil>", value: nil})
//
//	for i, v := range p1 {
//		for j, w := range p1 {
//			eq := getEqual(v, w)
...
func TestName(t *testing.T) {
	//x := []string{"/family/children/list/.*",}
	ignoreRules := []string{"/family/children/list/[\\d]*?/.*?"}

	obj_l, err := isRegexpContain(ignoreRules, "/family/children/list/0/name")
	//obj_l, err := getJson(JSON4)
	fmt.Println(err)
	fmt.Println(obj_l)
}